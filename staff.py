    ...
    try:
        ...
        with open(file, 'w') as json_file:
            json.dump(listObj, json_file, 
                                indent=4,  
                                separators=(',',': '))
        print(f"System Message: Car with plate {plateNumber} entered the parking")
...
def carExit():
    ...
    checker = plateValidator(userInput)
    if checker == None:
...
def plateValidator(plateNumber):
...
def main(userInput):
    ...
    if userInput == "1":
        ...
        checker = plateValidator(plateNumber)
        if checker == None:
            print("System Message: Invalid Plate Format")
        else:
